...
formulas=("git-flow-avh" "ruby" "node")
taps=("caskroom/cask")
casks=("java" "docker")
...
bootstrap() {
  if [ "${WASABI_OS}" == "${WASABI_OSX}" ]; then
    ...
    sudo npm i --registry=https://registry.npm.taobao.org install -g bower
    sudo npm i --registry=https://registry.npm.taobao.org install -g grunt-cli
    sudo npm i --registry=https://registry.npm.taobao.org install -g yo
